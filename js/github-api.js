...
class GitHubAPI {
    ...
    /**
     * Deleta o arquivo database.json do GitHub
     */
    static async deleteDatabase() {
        try {
            ...
            if (!getResponse.ok) {
                console.log('Arquivo não existe para deletar');
                return;
            }
            ...
            const deleteResponse = await fetch(
                `${this.BASE_URL}/repos/${config.githubUser}/${config.githubRepo}/contents/${this.DB_FILE}`,
                {
                    ...
                    body: JSON.stringify({
                        message: 'Deletar arquivo corrompido',
                        sha: sha,
                        branch: 'main'
                    })
                }
            ...
            if (deleteResponse.ok) {
                ...
                console.log('Erro ao deletar:', await deleteResponse.json());
            }
        } catch (error) {
     ...
     */
    static async updateDatabase(content, message = 'Atualizar estoque', retryCount = 0) {
        const MAX_RETRIES = 3;
        ...
        try {
            ...
            try {
                ...
                if (getResponse.status === 200 && getResponse.ok) {
                    const getBody = await getResponse.json();
                    if (getBody.sha) {
                        sha = getBody.sha;
                        console.log('✅ SHA obtido:', sha);
                    } else {
                        console.warn('⚠️ Resposta 200 mas sem SHA. Objeto:', getBody);
                    }
                } else if (getResponse.status === 404) {
                    console.log('ℹ️ Arquivo não existe - será criado');
                    sha = null;
                } else {
                    console.warn(`⚠️ Status inesperado ao obter SHA: ${getResponse.status}`);
                    sha = null;
                }
            } catch (err) {
                console.log('ℹ️ Erro ao obter SHA (arquivo novo):', err.message);
                sha = null;
            }
            ...
            // Se temos SHA, adiciona ao payload
            if (sha) {
                payload.sha = sha;
                console.log('📤 Enviando com SHA...');
            } else {
                console.log('📤 Enviando SEM SHA (arquivo novo)');
            }
            ...
            if (!response.ok) {
                ...
                console.error('❌ Erro do GitHub (status ' + response.status + '):', errorData);
                
                // Se erro é por SHA faltante e ainda temos retries
                if (errorData.message && errorData.message.includes('sha') && retryCount < MAX_RETRIES) {
                    console.log(`⚠️ GitHub reclama que SHA está faltando. Tentativa ${retryCount + 1}/${MAX_RETRIES}`);
                    console.log('🔄 Tentando deletar arquivo e recriar...');
                    
                    try {
                        await this.deleteDatabase();
                    } catch (delErr) {
                        console.log('Erro ao deletar (ignorando):', delErr.message);
                    }
                    
                    // Aguarda um pouco e tenta novamente
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    return this.updateDatabase(content, message, retryCount + 1);
                }
                
                throw new Error(`Erro ao atualizar: ${errorData.message}`);
            }
