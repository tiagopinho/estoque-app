...
class GitHubAPI {
    ...
    static async updateDatabase(content, message = 'Atualizar estoque') {
        try {
            ...
            // Primeiro tenta obter SHA fazendo GET direto para metadata
            try {
                ...
                if (getResponse.ok) {
                    ...
                    // Extrai SHA - pode estar em getBody.sha ou ser um array
                    if (getBody.sha) {
                        sha = getBody.sha;
                        console.log('SHA obtido do GET:', sha);
                    } else if (Array.isArray(getBody) && getBody[0] && getBody[0].sha) {
                        sha = getBody[0].sha;
                        console.log('SHA obtido do array:', sha);
                    }
                }
            } catch (err) {
                console.log('Erro ao GET SHA:', err.message);
            }
            ...
            if (sha) {
                payload.sha = sha;
                console.log('Enviando com SHA:', sha);
            } else {
            ...
            if (!response.ok) {
                ...
                console.error('Erro do GitHub:', errorData);
                
                // Se erro é por SHA faltante e arquivo existe, tenta uma última coisa
                if (errorData.message && errorData.message.includes('sha')) {
                    console.log('⚠️ GitHub reclama que SHA está faltando.');
                    console.log('💡 Solução: Delete database.json do repositório e tente novamente.');
                    console.log('📍 https://github.com/tiagopinho/estoque-app');
                }
