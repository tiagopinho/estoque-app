...
class App {
    ...
    static async saveProduct(e) {
        ...
        try {
            ...
            document.querySelectorAll('.validity-date').forEach(input => {
                if (input.value) validities.push(input.value);
            });
            ...
            if (Storage.isConfigValid()) {
                await this.syncWithGitHub('add', product.name);
            }
        } catch (error) {
    ...
    static async consumeProduct(productId) {
        try {
            ...
            // Sincroniza
            if (Storage.isConfigValid()) {
                await this.syncWithGitHub('consume', product.name, 1);
            }
        } catch (error) {
    ...
    static async deleteProduct(productId) {
        ...
        try {
            ...
            // Sincroniza
            if (Storage.isConfigValid()) {
                await this.syncWithGitHub('remove', product.name);
            }
        } catch (error) {
    ...
    static async registerConsumption(e) {
        ...
        try {
            ...
            const quantity = parseInt(UI.elements.consumptionForm.elements.quantity.value);
